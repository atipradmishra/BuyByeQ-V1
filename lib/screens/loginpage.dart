...
class LoginScreen extends StatelessWidget {
  ...
  Future<String?> _authUser(LoginData data) async {
    ...
    final result = await db.query(
      ...
      whereArgs: [data.name, data.password],
      columns: ['UserId'], // add the UserId column to the query
      limit: 1,
    );
    if (result.isEmpty) {
      return null;
    ...
    await db.update(
      ...
      {'IsActive': 1},
      where: 'UserId = ?',
      whereArgs: [result[0]['UserId']],
    );
    // Return the UserId value for the authenticated user
    return null;
  }
